...
export default function Edit({ auth, purchaseReceiving, suppliers, branches, products }) {
    const isPoLinked = !!purchaseReceiving.purchase_order_id;
    ...
    const { data, setData, put, processing, errors } = useForm({
        supplier_id: purchaseReceiving.supplier_id,
        branch_id: purchaseReceiving.branch_id,
        purchase_order_id: purchaseReceiving.purchase_order_id || '',
        received_at: purchaseReceiving.received_at,
        delivery_ref_number: purchaseReceiving.delivery_ref_number || '',
        notes: purchaseReceiving.notes || '',
        lines: purchaseReceiving.lines.map(line => ({
            id: line.id,
            purchase_order_line_id: line.purchase_order_line_id || '',
            product_id: line.product_id,
            sku: line.product?.sku || '',
            name: line.product?.name || '',
            ordered_quantity: line.ordered_quantity ? parseFloat(line.ordered_quantity) : 0,
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('procurement.receivings.update', purchaseReceiving.id));
    };
        ...
        <AuthenticatedLayout
            ...
            header={
                <div className="flex items-center gap-3">
                    <Link
                        href={route('procurement.receivings.show', purchaseReceiving.id)}
                    ...
                    <div>
                        <h2 className="font-extrabold text-2xl text-slate-800 leading-tight tracking-tight">
                            Edit Goods Receiving Voucher Draft
                        </h2>
                        <p className="text-sm text-slate-500 font-medium mt-1">
                            Modify quantities, lots, expiry dates, or unit costs before final posting.
                        </p>
                    </div>
                </div>
            }
        >
            <Head title="Edit Goods Receiving Voucher" />
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        disabled={branches.length === 1 || isPoLinked}
                                    >
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="bg-slate-50/50">
                                            ...
                                            {!isPoLinked && <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400 text-right w-[60px]"></th>}
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-50">
                                        {data.lines.length > 0 ? (
                                            data.lines.map((line, idx) => (
                                                <tr key={idx} className="hover:bg-slate-50/10">
                                                    ...
                                                    <td className="px-6 py-4 text-center text-sm font-semibold text-slate-400">
                                                        {isPoLinked ? line.ordered_quantity : '-'}
                                                    </td>
                                            ...
                                            <tr>
                                                <td colSpan={isPoLinked ? "8" : "9"} className="px-6 py-12 text-center text-sm font-medium text-slate-400">
                                        ...
                                        {data.lines.length > 0 && (
                                            <tr className="bg-slate-50/50">
                                                <td colSpan={isPoLinked ? "7" : "7"} className="px-6 py-4 text-right text-xs font-black uppercase text-slate-400 tracking-wider">
                            ...
                            <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm flex flex-col justify-between items-stretch">
                                <div>
                                    ...
                                    <div className="mt-6 flex items-start gap-2 bg-amber-50 rounded-xl p-3 border border-amber-100 text-[10px] text-amber-800 font-semibold leading-relaxed">
                                        ...
                                        <span>Saving this voucher keeps it as a DRAFT workspace. Real inventory stock and WAC values are updated only upon posting.</span>
                                    </div>
                                </div>
